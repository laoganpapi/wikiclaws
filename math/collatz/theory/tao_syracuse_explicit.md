...
>
> **Additional warning (a negative finding).** A second, *sharper* honest point emerges in §6.4 from the Step-1 numerics: the hypothesis $\mathrm{MIX}(\theta)$ is **strictly stronger** than Tao's $\beta=1$ conjecture (not equivalent), and the Syracuse collision probability shows that the distribution does **not** approach uniform in total variation at small $n$ — for *either* the untilted *or* the drift-tilted law (the tilt makes it worse at $n\le6$). So this document does not merely leave $\mathrm{MIX}(\theta)$ "open and plausible"; it presents *evidence raising doubt* about whether the natural-density upgrade holds at all via this route. That ambivalence is the honest state of the art after this work, and is itself the contribution: the gap is now *quantified* and a concrete reason it might be a true obstruction (not just a missing technique) is on the table.
...
> $$
> *where $s^*$ is the descent-balance tilt of (5.2). By Plancherel (Lemma 6.1) this implies the tilted distribution is within $\ell^2$-distance $\le C\,3^{-\theta n}$ of uniform on $(\ZZ/3^n\ZZ)^\times$, hence (for $\theta>\tfrac12$) within total-variation distance $\le\tfrac12 C\,3^{(\frac12-\theta)n}\to0$; and (for $\theta>\tfrac32$) its minimum mass obeys $c_n^{(s^*)}\ge \varphi(3^n)^{-1}(1-o(1))$. This is **strictly stronger** than Tao's "$\beta=1$" (which is only $c_n=3^{-n+o(n)}$, an exponential-**order** statement): $\mathrm{MIX}(\theta)$ demands the exact leading constant and an exponential **rate**. See §6.4 for why the distinction is non-vacuous.*
...
> **Numerical reality check (Step-1) — and a cautionary finding.** `verify_syracuse_rv.py` computes $\sup_{3\nmid\xi}|\widehat\nu_n(\xi)|$ for the *untilted* law: $0.577, 0.378, 0.252, 0.177, 0.129, 0.096, 0.076$ for $n=1,\dots,7$. It *decreases*, which looks encouraging — but the **per-step ratio is rising** ($0.65, 0.67, 0.70, 0.73, 0.74, 0.79$), drifting *above* $3^{-1/4}\approx0.76$ and away from $3^{-1/2}\approx0.58$. So the worst-case Fourier coefficient appears to decay with exponent $\theta\lesssim\tfrac14$ — i.e. **slower than the $\theta>\tfrac12$ that Lemma 6.2 needs.** This is examined properly via the collision probability in §6.4, where the picture sharpens into a genuine obstruction: for the *untilted* law, $\nu_n$ does **not** converge to uniform in TV at all. Whether the *tilt* $s^*$ repairs this is open (and is the key recommended experiment, §7).
...
> **Status of (5.3): OPEN — and strictly stronger than $\beta=1$.** The bound (5.3) — exponential decay of the (tilted) Syracuse characteristic function — is **not known**. It is *strictly stronger* than Tao's Proposition 1.17 (which gives only $n^{-A}$). It is **NOT equivalent** to Tao's $\beta=1$ conjecture; it is **strictly stronger** (this correction is forced by the §6.4 numerics — an earlier draft wrongly called them equivalent; see `dead_ends.md`). Precisely:
>
> **(B1) Relation to Tao's "$\beta=1$".** Tao 2020 (blog) conjectures $c_n=3^{-n+o(n)}$ ($\beta=1$), i.e. equidistribution at the *exponential order*. By Lemma 6.2, the natural-density upgrade needs equidistribution with the *exact leading constant* — $\varphi(3^n)\,\mathrm{CP}_n\to1$ (TV $\to0$), where $\mathrm{CP}_n$ is the collision probability. **$\beta=1$ does NOT imply this** (it permits a growing sub-exponential factor, $\varphi(3^n)\mathrm{CP}_n=3^{o(n)}\to\infty$); §6.4 exhibits exactly such growth for the untilted law. So $\mathrm{MIX}(\theta)$ $\Rightarrow$ $\beta=1$ but not conversely. Plain $\beta=1$ is open (problem **A.2**, `open_problems.md`); $\mathrm{MIX}(\theta)$ is strictly stronger and also open.
>
> **(B2) Power-saving cancellation for exponential sums over the $2$-power flow on $\ZZ/3^n\ZZ$.** Equation (6.3) is a $3$-adic exponential sum twisted by the geometric flow of $\times 2^{-1}$. Power-saving bounds for such "metaplectic"/"flow-twisted" sums on $\ZZ/p^n\ZZ$ are in general **open**; they sit in the same family as bounds for *short* exponential sums and *Kloosterman-type* sums over $p$-adic groups, where $p$-adically the available technology (stationary phase / Igusa local zeta functions / $p$-adic oscillatory integrals, cf. Igusa; Cluckers–Veys) gives clean power-saving for *fixed* algebraic phases but **not** for phases driven by an external geometric process with $n$-dependent suffix coupling. No unconditional power-saving for the specific sum (6.3) appears in the literature.

### 6.4 The collision-probability obstruction: $\beta=1$ is necessary but NOT sufficient

The correct quantity for the TV control in Lemma 6.2 is not the worst single Fourier coefficient but the **collision probability**
$$
\mathrm{CP}_n \;:=\; \sum_{b}\PP(\Syrac=b)^2 \;=\; \|\nu_n\|_{\ell^2}^2 \;=\; \frac{1}{3^n}\sum_{\xi}|\widehat\nu_n(\xi)|^2 .
$$
Since $\nu_n$ is supported on units, $\langle\nu_n,U\rangle=1/\varphi(3^n)$, so $\|\nu_n-U\|_{\ell^2}^2=\mathrm{CP}_n-1/\varphi(3^n)$, and by Lemma 6.1,
$$
\|\nu_n-U\|_{\mathrm{TV}} \;\le\; \tfrac12\sqrt{\varphi(3^n)}\,\|\nu_n-U\|_{\ell^2} \;=\; \tfrac12\sqrt{\varphi(3^n)\,\mathrm{CP}_n - 1} \;=\; \tfrac12\sqrt{E_n},\quad E_n:=\varphi(3^n)\,\mathrm{CP}_n-1. \tag{6.4}
$$
**TV $\to0$ (the natural-density requirement) is equivalent to $E_n\to0$**: equidistribution with the *exact leading constant*. By contrast, $\beta=1$ only asserts $\mathrm{CP}_n=3^{-n+o(n)}$, i.e. $E_n=3^{o(n)}$, which permits $E_n\to\infty$.

> **Finding (Step-1 numerics, `verify_syracuse_rv.py`, untilted law).** For the untilted ($s=0$) Syracuse distribution,
> $$
> E_n = \varphi(3^n)\,\mathrm{CP}_n - 1 \;=\; 0.111,\,0.429,\,0.736,\,1.046,\,1.356,\,1.667 \quad(n=1,\dots,6),
> $$
> with **constant successive differences $\approx 0.31$** — i.e. $E_n\approx 0.31\,n + \mathrm{const}$ grows **linearly**. So the untilted collision probability is $\mathrm{CP}_n=3^{-n}\cdot\mathrm{poly}(n)$ — exactly **$\beta=1$ order, with a linearly growing polynomial correction** — and consequently
> $$
> \|\nu_n-U\|_{\mathrm{TV}} \;\approx\; \tfrac12\sqrt{0.31\,n}\;\xrightarrow{n\to\infty}\;\infty .
> $$
> **The untilted Syracuse distribution does not converge to uniform in total variation; it drifts away (slowly).**

This is the sharpest honest statement of the bottleneck, and it is *stronger* than "the gap is open":

> **Proposition 6.3 (the natural-density upgrade needs more than $\beta=1$).** *For the untilted Syracuse law, the small-$n$ data is consistent with $\beta=1$ yet shows $E_n\to\infty$ (so TV $\not\to0$). Hence $\beta=1$ alone is provably insufficient for Conditional Theorem 5.2's hypothesis: $\mathrm{MIX}(\theta)$ requires the strictly stronger exact-leading-constant equidistribution $E_n^{(s^*)}\to0$ of the **tilted** law.*

> **Tilted finding (Step-1 numerics) — sobering and reported in full.** I computed $E_n^{(s^*)}$ for the descent-balance tilt $s^*\approx0.438$ as well:
> $$
> E_n^{(s^*)} = 0.212,\,0.854,\,1.772,\,3.090,\,4.972,\,7.643 \quad(n=1,\dots,6),
> $$
> with **increasing** successive differences ($0.64,0.92,1.32,1.88,2.67$): the tilted excess grows **super-linearly**, and at every $n\ge1$ it is **larger** than the untilted $E_n$. So at the finite-$n$ level the descent-balance tilt makes equidistribution *worse*, not better.
>
> **Why (mechanism, not excuse).** The tilt $s^*$ *thins* the geometric tail (lowers $\EE[a]$ from $2$ to $\log_23\approx1.585$), which concentrates the valuations $a_j$ onto smaller values, lowering the entropy of the multiplier $2^{-\sum a_j}$ and hence *raising* the collision probability. Mean-drift-neutrality (the property that makes natural-density sampling *stationary*) and high-entropy spreading (the property that makes the residue *equidistribute*) are **pulling in opposite directions** under this one-parameter tilt.
>
> **Honest consequence.** This is a real (negative) finding, not noise: it shows the naive hope "tilt to drift-neutrality $\Rightarrow$ equidistribution" **fails at small $n$**, and there is no evidence at $n\le6$ that $E_n^{(s^*)}\to0$. Three readings remain open, and I do not pretend to resolve them: **(i)** the small-$n$ regime is unrepresentative and $E_n^{(s^*)}$ eventually turns over (the asymptotic $3$-adic cancellation of §6.5, invisible at $n\le6$, could dominate later); **(ii)** the *correct* object is neither $s=0$ nor $s=s^*$ but a tilt that is itself $\xi$- or $n$-dependent, so the single Esscher parameter in §5.1 is too crude; **(iii)** natural density genuinely *fails* for this distribution and the log $\to$ natural upgrade is false as stated, which would be a major (negative) result pointing toward Vector B. **Distinguishing (i)/(ii)/(iii) is the single most important next step** (it needs $E_n$ to larger $n$, $\sim10$–$14$, via the FFT method below, and an exploration of $\xi$-dependent tilts).

### 6.5 Why the obstruction is real and not a packaging artifact
...
- **The drift coupling (Proposition 4.2) cannot be undone by the tilt alone at the level of bounds.** The tilt $s^*$ re-centers the *mean* drift to $0$, which is necessary for natural-density stationarity (and which §6.4 suggests is the *reason* the untilted $E_n$ grows), but it does not by itself produce *cancellation* in (6.3); it only changes the weight $w(\mathbf a)$. Cancellation still has to come from the oscillatory factor $e(\xi\Psi/3^n)$, i.e. from genuine $3$-adic equidistribution of $\Psi(\mathbf a)$ — which is (B2).
...
5. **Proposition 6.3 + the collision-probability finding (§6.4):** numerically discovered and structurally explained. The natural-density requirement is $E_n:=\varphi(3^n)\mathrm{CP}_n-1\to0$ (TV $\to0$), which is **strictly stronger than Tao's $\beta=1$** ($E_n=3^{o(n)}$ allowed). For the *untilted* law the Step-1 data shows $E_n\approx0.31n\to\infty$ (TV $\to\infty$): equidistribution at exponential order coexists with divergence in TV. **So $\beta=1$ is necessary but provably insufficient.** This sharpens the survey's framing (`survey.md` §10.2 spoke of "$L^2$ vs $L^\infty/$TV"; here that distinction is made quantitative and shown to be a genuine, non-vacuous gap, with the linear growth rate measured).

**The cleanest conditional theorem (the deliverable):**

> $\mathrm{MIX}(\theta)$ [exponential decay $3^{-\theta n}$, $\theta>\tfrac12$, of the **tilted** Syracuse characteristic function (5.3); equivalently $E_n^{(s^*)}\to0$ at a geometric rate] $\;\Longrightarrow\;$ Tao's theorem in **natural** density (Conditional Theorem 5.2).

**What $\mathrm{MIX}(\theta)$ reduces to, and its status:**

- It is **strictly stronger than Tao's $\beta=1$ conjecture** (`open_problems.md` A.2): it is an *exact-leading-constant*, *drift-tilted*, *exponential-rate* equidistribution. Plain $\beta=1$ is open; $\mathrm{MIX}(\theta)$ is strictly stronger and open. (Corrected from an earlier "equivalent to tilted $\beta=1$" claim — see §6.4 and `dead_ends.md`.)
- Equivalently it is a **power-saving bound for the $3$-adic exponential sum (6.3)** twisted by the geometric flow of multiplication-by-$2^{-1}$ on $(\ZZ/3^n\ZZ)^\times$. No such unconditional bound is known; the obstruction (skew convolution + drift coupling, §6.5) is structural, not a packaging artifact.

**The honest size of the remaining gap.** Large, but *precisely located*:

1. **Analytic core (the hard $\sim$90%).** Prove $E_n^{(s^*)}\to0$ at a geometric rate, equivalently exponential cancellation in (6.3). This is genuinely open and is *harder* than Tao's $\beta=1$; nothing here lowers that wall. I did **not** make progress on the cancellation itself. The contribution is to show *exactly how much* equidistribution is needed (exact leading constant, not merely exponential order) and that it must be of the *tilted* law.
2. **Transport-machinery core (the remaining $\sim$10%, but real).** Steps A and C of Conditional Theorem 5.2 reproduce Tao's iteration under the tilted natural-density measure. I have given the architecture and the Esscher/tilt mechanism, but **not** a formalization-ready proof; a faithful reproduction of Tao's first-passage iteration under the tilt is required, and a red-team pass against the actual paper (Step 2 of the protocol) is mandatory before any of Theorem 5.2's non-Lemma-6.2 steps are trusted.
3. **A sobering partial result on the decisive experiment.** I computed $E_n^{(s^*)}$ for the tilted law (§6.4): it grows *super-linearly* and is *worse* than untilted at every $n\le6$ — the descent-balance tilt does **not** rescue equidistribution at small $n$. This does **not** refute Conditional Theorem 5.2 (its hypothesis is asymptotic; small-$n$ growth can reverse), but it removes the easy optimism: there is currently **no numerical evidence** that $\mathrm{MIX}(\theta)$ holds, and a live possibility (reading (iii) of §6.4) is that **natural density fails outright** for the Syracuse distribution, which would be a significant negative result redirecting toward Vector B. The honest status of $\mathrm{MIX}(\theta)$ is therefore: *open, strictly stronger than $\beta=1$, and not supported by the available small-$n$ data*. Next: push $E_n$ to $n\sim10$–$14$ (FFT-based, $\widehat\nu_n$ via the skew-convolution (2.2) in $O(n\,3^n)$), and test $\xi$-dependent tilts.

**Bottom line for the brief's four questions** is recorded in the agent report; in one line: *the log→natural gap reduces, with the exact required strength identified (exact-leading-constant equidistribution of the drift-tilted Syracuse law, $E_n^{(s^*)}\to0$; equivalently power-saving $3^{-\theta n}$ Fourier decay, $\theta>\tfrac12$), to an exponential-sum bound that is **strictly stronger** than Tao's open $\beta=1$ conjecture — and that bound remains open, with a numerically-exhibited reason ($\beta=1$ gives only $E_n=3^{o(n)}$, and the untilted law has $E_n\to\infty$) for why $\beta=1$ alone cannot suffice and why the drift tilt is essential.*

---

## 8. Verification-protocol status

- **Step 1 (computational sanity):** PASSED for the *definitional* content — `experiments/verify_syracuse_rv.py` reproduces the $n=1$ Syracuse law exactly, confirms support on units, confirms submultiplicativity (2.3) on $n\le5$, confirms the tilt $s^*$ ($\EE_{s^*}[a]=\log_23$), and computes $c_n$, $c_n3^n$, $\sup|\widehat\nu_n|$, and the collision excess $E_n$ (§6.4, the linear-growth finding). Log at `experiments/verify_syracuse_rv_log.md`. The asymptotic claims (3.1)/(5.3) are *not* finitely checkable; for them Step 1 is "test-case construction," satisfied by the small-$n$ tables plus the structural derivation. The §6.4 finding that *untilted* $E_n\to\infty$ is a genuine (negative) Step-1 result, not just a sanity check.
- **Step 2 (red-team):** NOT done. Required, with emphasis on: (a) faithfulness of §5 Steps A/C to Tao's actual transport propositions (the secondary-source reconstruction must be checked against the paper); (b) correctness of the tilt value $s^*$ and the Esscher step; (c) the claim that $\mathrm{MIX}(\theta)$ is strictly stronger than Prop 1.17 and **strictly stronger** than $\beta=1$ (NOT equivalent — §6.4); (d) independent recomputation of $E_n$ and, if feasible, the tilted $E_n^{(s^*)}$.
