...
>
> **Headline finding (stated up front so it is not buried).** The rate-limiting Diophantine input is **NOT the one-dimensional irrationality measure $\mu(\log_2 3)$**, contrary to the framing in `survey.md` §7.5 / §11 Vector C and `open_problems.md` D.1/D.2. The Steiner–Simons–de Weger–Hercher proofs obtain the upper bound on the cycle length from a **lower bound for a linear form in *two* logarithms**, $\Lambda = (K+S)\log 2 - K\log 3 > 0$, supplied by the **Laurent–Mignotte–Nesterenko (LMN)** theorem and Laurent's interpolation-determinant method (verified: the upper bound on cycle length "is found by applying a result of Laurent, Mignotte, and Nesterenko"; the lower bound is Crandall's lemma). The cleanest packaged form is de Weger's, verified verbatim: *$0<(K+S)\log2-K\log3<2^{-0.158K}$ has no solution for $K\ge32$* — exponent **linear in $K$**, downstream of LMN's constant $24.34\,D^4$. **The constant that bounds $m^\*$ is therefore the two-log estimate's leading constant ($24.34\,D^4$ / de Weger exponent $0.158$), together with the circuit-averaging budget and the verified bound $B$ — not $\mu(\log_2 3)$, which the published method never uses.** (I give in §5.2 the intuition for why a one-dimensional measure is the wrong tool, but flag it `[CLAIM-UNVERIFIED]` rather than assert it as proven.) This is the single most important deliverable of this document; it is developed in §5–§6 and corrects the survey/open-problems framing.
...
### 5.2 Why the two-log estimate is the bottleneck — and why the published method does not use $\mu(\log_2 3)$

**What is solidly established (from the literature, verified):** the Simons–de Weger–Hercher proofs obtain the upper bound $K < F(m)$ by applying a **two-log linear-forms estimate** (Laurent/LMN) — *not* the one-dimensional irrationality measure $\mu(\log_2 3)$. The constant that governs $F(m)$, and hence $m^\*$, is the **two-log estimate's leading constant** ($24.34\,D^4$; de Weger exponent $0.158$). Improving $m^\*$ therefore means improving the **two-log estimate**, the **circuit-averaging budget**, or the **verified bound $B$** — *not* $\mu(\log_2 3)$. This much is a direct reading of the proofs and is the operative correction to `survey.md` §11 / `open_problems.md` D.1–D.2 (which frame the bottleneck as $\mu(\log_2 3)$).

**Why the two-log estimate, and not $\mu(\log_2 3)$ — an intuition, flagged `[CLAIM-UNVERIFIED]`.** A lower bound on the *two-log form* $\Lambda$ uses the heights of $2$ **and** $3$ separately and is correspondingly sharper than what the *ratio's* measure gives. Heuristically:

- The two-log estimate $\Lambda > 2^{-0.158K}$ has an exponent **linear in $K$**. Combined with the cycle's forcing that $\Lambda$ be very small, this yields a strong, essentially $K$-uniform constraint — the source of the finite $F(m)$.
- The irrationality measure gives only $\Lambda > (\log 2)K^{1-\mu}$, a **polynomial** lower bound. Whether (and how strongly) a polynomial bound could substitute for the two-log estimate in producing $F(m)$ is **not obvious and I have not rigorously settled it** — the naive comparison is delicate because the cycle's own forcing on $\Lambda$ is itself $K$-dependent. What is clear is (i) the published proofs do **not** use $\mu(\log_2 3)$, and (ii) the literature is explicit that the *upper bound on cycle length comes from LMN*. I therefore do **not** claim "improving $\mu$ is provably useless"; I claim the **lever the proofs actually pull is the two-log estimate**, and that is where improvement effort should go.

> **Conclusion (the correction, stated at the right confidence).** The bottleneck for cycle exclusion is the **two-log linear-forms estimate** (Laurent/LMN), via its leading constant $24.34\,D^4$ / de Weger exponent $0.158$ — **not** the one-dimensional irrationality measure $\mu(\log_2 3)$, which the published method does not use. `survey.md` §7.5/§11 and `open_problems.md` D.1/D.2 frame the bottleneck as $\mu(\log_2 3)$; that is at best imprecise and should be corrected to "the two-log linear-forms estimate." Whether a future argument could route the bound through $\mu(\log_2 3)$ is open; the *current* lever is the two-log constant, the circuit budget, and $B$.
...
1. **A larger verified $B$.** Hercher uses $B = 3\cdot 2^{69}$. Barina 2025 reaches $2^{71}$ (`survey.md` §6.1). Re-running with $B = 2^{71}$ tightens Crandall's lower bound (§2 eq (6)) and would raise $m^\*$ by a small amount (each doubling of $B$ adds a little). **This is the cheapest concrete gain and is almost certainly worth ~1–3 in $m^\*$.** (Hercher's "$m\ge 83$ with newer verification" remark shows the verification bound alone moves $m^\*$ by several units.)
...
| $\mu(\log_2 3)$ | via $\mu(\log 3)\le 5.116$ (Wu–Wang 2014) | **none in the published method** | Not used by Steiner–SdW–Hercher (§5.2); corrects `open_problems.md` D.1/D.2. (A $\mu$-based route is not obviously possible — `[CLAIM-UNVERIFIED]`.) |
| *Conjectural:* $\mu(\log_2 3) = 2+\epsilon$ | conjectural | **none in the published method** | Same: the method's lever is the two-log estimate, not the ratio's measure. |
...
**Conclusion on the conjectural side:** The bound that the *published method* would leverage to push $m^\*$ much higher is a **sharper lower bound for the two-log linear form** $|N\log 2 - K\log 3|$ — concretely the conjectural Lang–Waldschmidt two-dimensional bound $|\Lambda| \gg H^{-1-\epsilon}$ ($H\sim\max(N,K)$), far beyond current transcendence technology (current exponent $\sim(\log H)^2$; conjectural $(1+\epsilon)\log H$). A better one-dimensional $\mu(\log_2 3)$ is **not** what the method uses (§5.2); whether some *other* argument could exploit $\mu$ is open, but for the Steiner–Simons–de Weger–Hercher line the lever is unambiguously the two-log estimate. Even the conjectural two-log bound only sharpens the *exponential* decay rate, pushing $m^\*$ very high but not, by itself, to "$\Lambda$ bounded below by a constant," so it would not rule out cycles of *all* $m$. **Unbounded cycle exclusion ($m^\* = \infty$) is exactly conditional on a strong-enough two-log lower bound.**
...
2. **`survey.md` §7.5, §11 Vector C, `open_problems.md` D.1/D.2:** "the bottleneck is the effective irrationality measure of $\log_2 3$" is **incorrect / imprecise**. The bottleneck in the *published* proofs is the **two-log linear-forms estimate** (LMN/Laurent), via its leading constant $24.34\,D^4$ (de Weger exponent $0.158$); the irrationality measure $\mu(\log_2 3)$ does not appear in the method. Recommend rewording D.1/D.2 and Vector C to "requires a sharper two-log linear-forms estimate (Laurent/LMN), or a larger verified bound $B$, or sharper circuit averaging." (Whether a $\mu$-based route is *possible in principle* is left open; see §5.2 `[CLAIM-UNVERIFIED]`.) This is the central correction.
