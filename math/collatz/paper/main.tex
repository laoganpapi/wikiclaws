...
\[
  r_C^{\,2}\;=\;\frac{2\log_2 3 -3}{2\log_2 3+1}\;\approx\;0.04075,
  \qquad
...
Under the Class~(C) tilt at $(s_C,t_C)$ of Proposition~\ref{prop:classC}, the induced mod-$9$ marginal
of $X_n^{(s_C,t_C)}$ on the units of $\Z/9\Z$ has
\[
  \TV\!\Big(\pi_*\nu_n^{(s_C,t_C)},\,U_{(\Z/9\Z)^\times}\Big)\;\approx\;0.313,
\]
\emph{worse} than the untilted mod-$9$ TV ($1/3$) is essentially saturated and worse than the
single-parameter Esscher mod-$9$ TV ($\approx 0.152$). The within-coset distribution of $X_n\bmod 9$
within each mod-$3$ class is highly non-uniform: numerically (cf.\ project data) the six unit residues
$\{1,2,4,5,7,8\}\bmod 9$ carry masses
$(0.250,\,0.250,\,0.240,\,0.010,\,0.010,\,0.240)$ respectively,
versus the uniform target $1/6\approx 0.167$ each.
\end{proposition}
...
%  stability under truncation cross-checked at amax=400.]
\end{proof}
