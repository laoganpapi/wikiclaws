...
and joint LDP rate $\approx 0.228=4.15\times I(0)$; and a finite-dimensional barrier that obstructs
iteration of this saturation to mod-$3^k$ for $k>1$.
...
The mod-$9$ failure is structural, not a numerical artifact. It is shadowed by a clean
dimension-counting barrier:

\begin{remark}[finite-$d$ obstruction to iterating saturation]\label{rem:dimbarrier}
...
tilt. A translation-invariant per-coord tilt that depends on a fixed-dimensional family of cocycles of
the form $\psi_1,\dots,\psi_d:\nat\to\R$ has $d$ free parameters; once $d<2\cdot 3^{k-1}-1$ the
constraint system is generically overdetermined. Class~(C) has $d=2$ (the cocycles $\varphi$ and
$\psi$) and saturates mod-$3$ ($d\ge \tot(3)-1=1$) but provably fails at mod-$9$
($d=2 < \tot(9)-1=5$). To saturate mod-$3^k$ within translation-invariant per-coord tilts of bounded
dimension requires either (i) growing $d$ as $\Theta(3^k)$ with $k$, or (ii) using block cocycles
$\psi_k(a_{j-k+1},\dots,a_j)$ that couple consecutive coordinates. Option~(i) is no longer ``thermodynamic
formalism'' but explicit residue conditioning, and degenerates to a Radon--Nikodym restatement of the
target. Option~(ii) is the live frontier, with $d=5$ (block cocycles of length $\le 2$) at mod-$9$ the
first open question.
% [NOVELTY UNVERIFIED --- dimension-counting obstructions are standard in thermodynamic formalism / large deviations;
%  the explicit packaging for the Syracuse mod-$3^k$ marginal stream is, to our knowledge, not previously recorded.]
\end{remark}
...
(iv) a clean finite-dimensional barrier --- empirical at mod-$9$ (Proposition~\ref{prop:mod9}),
heuristic at mod-$3^k$ (Remark~\ref{rem:dimbarrier}) --- that translation-invariant per-coord tilts
of fixed dimension cannot saturate the entire mod-$3^k$ residue tower required to close the
log~$\to$~natural gap.
