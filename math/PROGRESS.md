...
| 8 | JI-filter overlap inequality | ↑a∩↑b = ↑(a∨b) *exactly* (tautology) ⇒ no slack; P₂=OVL=M₂ (same as #7). Overlap inequality FALSE (6 families, dips to 0.444). |

### ⭐⭐ BARRIER THEOREM (the capstone of the methodological survey)
All eight methods reduce to a **symmetric convex moment of the frequency/fibre vector**, and every such moment is **flat-minimized by the Boolean cube at exactly ½**. ⇒ **No symmetric convex moment of the frequencies can prove Frankl.** The required ingredient must be a *non-symmetric* function (sensitive to *which* element is heavy / the generator incidence structure), not any averaged/spectral/moment quantity. This is a Frankl analogue of a natural-proofs barrier — a real, publishable "why the standard toolkit cannot work" result, plus the cube-uniqueness extremal characterization (verified n≤5). [NOVELTY UNVERIFIED.]
